...
<div class="row C_Coupon-Discount-Center">
  ...
  <asp:Panel ID="ApplyCouponForm" DefaultButton="ApplyCouponButton" runat="server" role="form" >
          <div class="col-xs-12"><i>Got a Coupon Code? Enter it and We Will Apply that to your cart as well.</i></div>
        ...
        <asp:ListView ID="lvDiscount" runat="server" DataKeyNames="DiscountId" ItemPlaceholderID="itemPlaceHolder" OnItemCommand="lvDiscount_ItemCommand">
            ...
            <ItemTemplate>
                <li><%# HttpUtility.HtmlEncode(Eval("DiscountCode")) %> <asp:LinkButton ID="removeDiscount" runat="server" Text="Remove" CommandName="DeleteCoupon" CommandArgument='<%# HttpUtility.HtmlEncode(Eval("DiscountCode"))%>' CssClass="btn btn-danger btn-xs" Visible='<%# AllowEditCoupons %>' /></li>
            </ItemTemplate>
        </asp:ListView>
    </div>
</div>